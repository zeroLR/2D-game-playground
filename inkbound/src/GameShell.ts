...
export class GameShell extends Container {
  ...
  private readonly attackFx = new Graphics();
  private readonly trailLayer = new Container();
  private readonly camera = new CameraController();
  private readonly motion = createPlayerMotion();
  private readonly attack = createAttackState();
  private readonly enemies = [new Enemy(1, 790, 500, 705, 885), new Enemy(2, 1010, 455, 960, 1065)];
  private dashWasPressed = false; private attackWasPressed = false; private trailTimer = 0; private hitStop = 0;

  constructor() {
    super();
    const gameplayMask = new Graphics().rect(0, 0, LOGICAL_WIDTH, GAMEPLAY_HEIGHT).fill(0xffffff);
    this.addChild(this.world, gameplayMask); this.world.mask = gameplayMask;
    this.drawWorld(); this.world.addChild(this.trailLayer);
    for (const enemy of this.enemies) this.world.addChild(enemy);
    this.drawPlayer(); this.world.addChild(this.attackFx); this.drawHud(); this.addChild(new ControlDeck(this.input));
  }
  ...
  update(deltaSeconds: number) {
    ...
    const dashPressed = input.dash && !this.dashWasPressed; const attackPressed = input.attack && !this.attackWasPressed;
    this.dashWasPressed = input.dash; this.attackWasPressed = input.attack;
    if (attackPressed) startAttack(this.attack);
    stepAttack(this.attack, dt); stepPlayer(this.motion, input.moveX, dashPressed, dt, PLATFORMS);
    for (const enemy of this.enemies) enemy.update(this.motion.x, dt);
    this.resolveAttack(); this.camera.update(this.motion.x, this.motion.y, this.motion.facing, dt);
    this.player.position.set(this.motion.x, this.motion.y - 25); this.player.scale.x = this.motion.facing;
    this.world.position.set(-this.camera.x, -this.camera.y); this.drawAttackFx();
  ...
  private resolveAttack() {
    if (!isAttackActive(this.attack)) return;
    const hitbox = playerAttackHitbox(this.motion.x, this.motion.y, this.motion.facing);
    for (const enemy of this.enemies) {
      if (enemy.hp <= 0 || this.attack.hitIds.has(enemy.enemyId) || !overlaps(hitbox, enemy.hurtbox())) continue;
      this.attack.hitIds.add(enemy.enemyId); enemy.hit(this.motion.facing); this.hitStop = HIT_STOP;
    }
  }

  private drawAttackFx() {
    this.attackFx.clear(); if (!isAttackActive(this.attack)) return;
    const x = this.motion.x + this.motion.facing * 34; const y = this.motion.y - 27;
    this.attackFx.arc(x, y, 35, this.motion.facing > 0 ? -1.05 : 2.1, this.motion.facing > 0 ? 1.05 : 4.18).stroke({ color: 0x24231f, width: 6, alpha: 0.65 });
  }

  private drawWorld() {
    this.world.addChild(new Graphics().rect(0, 0, WORLD_WIDTH, WORLD_HEIGHT).fill(0xe9e4d8));

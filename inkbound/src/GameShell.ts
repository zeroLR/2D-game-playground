...
export class GameShell extends Container {
  ...
  private readonly world = new Container(); private readonly player = new Container(); private readonly attackFx = new Graphics();
  private readonly impactLayer = new Container(); private readonly trailLayer = new Container(); private readonly camera = new CameraController();
  private readonly motion = createPlayerMotion(); private readonly attack = createAttackState();
  private readonly enemies = [new Enemy(1, 790, 500, 705, 885), new Enemy(2, 1010, 455, 960, 1065)];
  private dashWasPressed = false; private attackWasPressed = false; private trailTimer = 0; private hitStop = 0; private shakeTime = 0;

  constructor() {
    super(); const gameplayMask = new Graphics().rect(0, 0, LOGICAL_WIDTH, GAMEPLAY_HEIGHT).fill(0xffffff);
    this.addChild(this.world, gameplayMask); this.world.mask = gameplayMask; this.drawWorld(); this.world.addChild(this.trailLayer);
    for (const enemy of this.enemies) this.world.addChild(enemy);
    this.drawPlayer(); this.world.addChild(this.attackFx, this.impactLayer); this.drawHud(); this.addChild(new ControlDeck(this.input));
  }

  update(deltaSeconds: number) {
    const dt = Math.min(deltaSeconds, 1 / 30);
    this.updateImpactFx(dt);
    if (this.hitStop > 0) { this.hitStop = Math.max(0, this.hitStop - dt); this.applyCameraPosition(); return; }
    const input = this.input.snapshot(); const dashPressed = input.dash && !this.dashWasPressed; const attackPressed = input.attack && !this.attackWasPressed;
    this.dashWasPressed = input.dash; this.attackWasPressed = input.attack; if (attackPressed) startAttack(this.attack);
    stepAttack(this.attack, dt); stepPlayer(this.motion, input.moveX, dashPressed, dt, PLATFORMS); for (const enemy of this.enemies) enemy.update(this.motion.x, dt);
    this.resolveAttack(); this.camera.update(this.motion.x, this.motion.y, this.motion.facing, dt); this.player.position.set(this.motion.x, this.motion.y - 25); this.player.scale.x = this.motion.facing;
    this.applyCameraPosition(); this.drawAttackFx();
  ...
  private spawnImpact(x: number, y: number) {
    const burst = new Graphics().circle(0, 0, 7).stroke({ color: 0x171714, width: 5, alpha: 0.9 });
    for (let i = 0; i < 6; i++) { const a = (Math.PI * 2 * i) / 6; burst.moveTo(Math.cos(a) * 8, Math.sin(a) * 8).lineTo(Math.cos(a) * 25, Math.sin(a) * 25).stroke({ color: 0x171714, width: 3, alpha: 0.75 }); }
    burst.position.set(x, y); this.impactLayer.addChild(burst);
  }

  private updateImpactFx(dt: number) {
    this.shakeTime = Math.max(0, this.shakeTime - dt);
    for (const child of [...this.impactLayer.children]) { child.alpha -= dt * 5.5; child.scale.set(child.scale.x + dt * 2.2); if (child.alpha <= 0) child.destroy(); }
  }
  ...
  private drawWorld() {
    this.world.addChild(new Graphics().rect(0, 0, WORLD_WIDTH, WORLD_HEIGHT).fill(0xe9e4d8)); const farMountains = new Graphics();
    for (let x = -100; x < WORLD_WIDTH + 200; x += 280) farMountains.moveTo(x, 390).bezierCurveTo(x + 80, 300, x + 135, 335, x + 210, 265).bezierCurveTo(x + 270, 340, x + 320, 330, x + 380, 395).lineTo(x + 380, 560).lineTo(x, 560).fill({ color: 0x77756c, alpha: 0.10 }); this.world.addChild(farMountains);
    ...
    const hint = new Text({ text: '斬 = 近戰  ·  身 = Dash  ·  向右尋找墨獸', style: { fill: 0x777269, fontSize: 13 } }); hint.position.set(42, 365); this.world.addChild(hint);
  }
  private drawHud() { const hp = new Text({ text: '● ● ● ● ●', style: { fill: 0x3e3c35, fontSize: 14, letterSpacing: 2 } }); hp.position.set(14, 14); const ink = new Text({ text: '墨  ◐ ◐ ◐', style: { fill: 0x3e3c35, fontSize: 14 } }); ink.position.set(286, 14); this.addChild(hp, ink); }

...
import { getRouteAccent } from './RouteTheme';

const META: Record<RouteKind, { label: string; detail: string; hint: string }> = {
  treasure: { label: 'TREASURE', detail: 'REWARD ROUTE', hint: 'ENTER FOR CRYSTALS' },
  elite: { label: 'ELITE', detail: 'HIGH PRESSURE', hint: 'ENTER FOR RISK' },
  rest: { label: 'REST', detail: 'RECOVERY ROUTE', hint: 'ENTER TO RECOVER' },
  skill: { label: 'SKILL', detail: 'BUILD CHOICE', hint: 'ENTER TO BUILD' },
};

export { getRouteAccent } from './RouteTheme';

export function createRouteVisual(kind: RouteKind): Container {
  const root = new Container();
  const meta = META[kind];
  const accent = getRouteAccent(kind);
  const zone = new Graphics();
  zone.roundRect(-ROUTE_ZONE_HALF_WIDTH, -ROUTE_ZONE_HALF_HEIGHT, ROUTE_ZONE_HALF_WIDTH * 2, ROUTE_ZONE_HALF_HEIGHT * 2, 18)
    .fill({ color: accent, alpha: 0.075 });
  zone.roundRect(-ROUTE_ZONE_HALF_WIDTH, -ROUTE_ZONE_HALF_HEIGHT, ROUTE_ZONE_HALF_WIDTH * 2, ROUTE_ZONE_HALF_HEIGHT * 2, 18)
    .stroke({ width: 1.2, color: accent, alpha: 0.28 });
  ...
  marker.moveTo(-13, 3).lineTo(0, 14).lineTo(13, 3).stroke({ width: 2, color: accent, alpha: 0.72 });
  marker.moveTo(-8, -5).lineTo(0, 2).lineTo(8, -5).stroke({ width: 1.5, color: accent, alpha: 0.42 });

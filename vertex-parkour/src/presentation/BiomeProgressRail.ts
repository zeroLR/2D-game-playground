...
export class BiomeProgressRail {
  ...
  constructor() {
    ...
    for (const [index, biome] of BIOME_SEQUENCE.entries()) {
      const label = new Text({
        text: LABELS[biome],
        style: new TextStyle({ fill: '#9eb8b5', fontSize: 6.5, fontWeight: '600', letterSpacing: 0.5 }),
      });
      label.position.set(RAIL_X + 10, START_Y + index * STEP_Y - 4);
      this.labels.set(biome, label);
  ...
  update(activeBiome: BiomeId) {
    ...
    const activeIndex = BIOME_SEQUENCE.indexOf(activeBiome);
    const endY = START_Y + (BIOME_SEQUENCE.length - 1) * STEP_Y;

    this.rail.clear();
    this.rail.moveTo(RAIL_X, START_Y).lineTo(RAIL_X, endY).stroke({ width: 1, color: 0x789b99, alpha: 0.18 });

    for (const [index, biome] of BIOME_SEQUENCE.entries()) {
      const theme = getBiomeTheme(biome);
      const completed = index < activeIndex;
      const active = index === activeIndex;
      const alpha = active ? 0.95 : completed ? 0.52 : 0.18;
      const radius = active ? 3.4 : 2.4;
      ...
      if (label) {
        label.style.fill = theme.platformTint;
        label.alpha = active ? 0.9 : completed ? 0.5 : 0.2;
      }
    }
  }
}

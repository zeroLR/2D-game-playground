...
const ANIMATION_SPEED: Record<NovaAnimationState, number> = {
  idle: 0.08,
  jump: 0.14,
  fall: 0.12,
  'dash-left': 0.62,
...
const LOOPING: Record<NovaAnimationState, boolean> = {
  idle: true,
  jump: false,
  fall: true,
  'dash-left': false,
...
export type NovaPlayerVisualFrame = {
  x: number;
  y: number;
  elapsed: number;
  velocityY: number;
...
/**
 * Owns Nova's presentation without changing gameplay state or collision shape.
 * The legacy vector player remains as a loading/error fallback so the player is
 * never invisible while browser textures are still being decoded.
 */
export class NovaPlayerRenderer {
  ...
  update(frame: NovaPlayerVisualFrame) {
    ...
    if (!this.sprite) {
      redrawPlayer(this.fallback, 0, 0, frame.elapsed, frame.dashVisualTime > 0 ? frame.dashDirection : 0);
      return;
    }
  ...
  private async loadTextures() {
    try {
      ...
      await Promise.all(states.map(async (state) => {
        const frames = await Promise.all(NOVA_FRAME_URLS[state].map((url) => Assets.load<Texture>(url)));
        this.textures.set(state, frames);
      }));
      ...
      this.sprite.scale.set(FRAME_SCALE);
      this.sprite.animationSpeed = ANIMATION_SPEED.idle;
      this.sprite.loop = true;
      this.fallback.visible = false;
      this.view.addChild(this.sprite);
      this.sprite.play();
    } catch (error) {
      console.warn('Nova animation textures failed to load; using vector fallback.', error);
    }
  }
  ...
  private play(state: NovaAnimationState) {
    ...
    this.sprite.animationSpeed = ANIMATION_SPEED[state];
    this.sprite.loop = LOOPING[state];

...
export function createEnvironment(width: number, height: number): EnvironmentLayers {
  ...
  for (let i = 0; i < 10; i += 1) {
    ...
    const w = 24 + (i % 3) * 12;
    const h = 120 + (i % 4) * 55;
    tower.rect(-w / 2, -h, w, h).fill({ color: Palette.distant, alpha: 0.3 });
    tower.poly([-w / 2, -h, 0, -h - 28 - (i % 2) * 18, w / 2, -h]).fill({ color: Palette.distant, alpha: 0.3 });
    if (i % 2 === 0) tower.rect(-3, -h + 34, 6, 22).fill({ color: Palette.teal, alpha: 0.08 });
  ...
  for (let i = 0; i < 7; i += 1) {
    ...
    const w = 40 + (i % 3) * 18;
    const h = 150 + (i % 4) * 70;
    ruin.rect(-w / 2, -h, w, h).fill({ color: Palette.mid, alpha: 0.44 });
    ruin.poly([-w / 2, -h, -w * 0.15, -h - 25, w * 0.12, -h - 10, w / 2, -h]).fill({ color: Palette.mid, alpha: 0.44 });
    ruin.rect(-w * 0.3, -h + 42, w * 0.6, 6).fill({ color: Palette.teal, alpha: 0.06 });
  ...
  for (let i = 0; i < 11; i += 1) {
    ...
    mote.circle(0, 0, size).fill({ color: i % 5 === 0 ? Palette.gold : Palette.tealSoft, alpha: 0.12 + (i % 4) * 0.035 });
...
export function createHazardVisual(): Graphics {
  const g = new Graphics();
  g.circle(0, 0, 11).fill(Palette.hazard);
  g.circle(0, 0, 14).stroke({ width: 1.5, color: Palette.magentaDeep, alpha: 0.45 });
  for (let i = 0; i < 8; i += 1) {
    const a = (Math.PI * 2 * i) / 8;
    const inner = 13;
    const outer = i % 2 === 0 ? 20 : 18;
    g.moveTo(Math.cos(a) * inner, Math.sin(a) * inner)
      .lineTo(Math.cos(a) * outer, Math.sin(a) * outer)
      .stroke({ width: 3.5, color: Palette.magentaDeep, alpha: 0.72 });
  }
  g.circle(0, 0, 4).fill({ color: 0xf16c78, alpha: 0.88 });
  g.circle(-1.5, -1.5, 1.1).fill({ color: Palette.cream, alpha: 0.45 });
  return g;
}

export function setHazardDanger(view: Graphics, danger: number) {
  const clamped = Math.max(0, Math.min(1, danger));
  view.scale.set(0.92 + clamped * 0.12);
  view.alpha = 0.68 + clamped * 0.32;
}

export function createCrystalVisual(): Graphics {
  const g = new Graphics();
  g.circle(0, 0, 20).fill({ color: Palette.tealSoft, alpha: 0.035 });
  g.poly([0, -16, 11, 0, 0, 16, -11, 0]).fill(Palette.teal);
  g.poly([0, -10, 6, 0, 0, 10, -6, 0]).fill(Palette.tealSoft);
  g.poly([0, -16, 11, 0, 4, -2, 0, -10]).fill({ color: 0xffffff, alpha: 0.18 });
  g.circle(-17, 7, 1.1).fill({ color: Palette.tealSoft, alpha: 0.22 });
  g.circle(15, -8, 0.8).fill({ color: Palette.tealSoft, alpha: 0.18 });
  return g;
}
...
export function redrawPlayer(view: Graphics, x: number, y: number, elapsed: number, dashing: number) {
  ...
  const lean = dashing === 0 ? 0 : dashing * 5;

  if (dashing !== 0) {
    view.poly([-28 * dashing, 2, -12 * dashing, -8, -8 * dashing, 6]).fill({ color: Palette.teal, alpha: 0.16 });
  }

  // Oversized hood creates the primary mascot silhouette.
  view.poly([-17 + lean, 8, -7 + lean, -24, 10 + lean, -22, 19 + lean, 7, 5 + lean, 14]).fill(Palette.cream);
  view.poly([-7 + lean, -24, 10 + lean, -22, 4 + lean, -9]).fill(0x172b2d);
  view.circle(0 + lean, -10, 2.3).fill(Palette.backgroundDeep);

  // Compact dark body keeps the hood dominant.
  view.roundRect(-7 + lean, 9, 15, 14, 5).fill(0x243d3f);
  view.poly([-7 + lean, 12, -23 - sway + lean, 18, -9 + lean, 7]).fill({ color: Palette.teal, alpha: 0.78 });
  view.poly([7 + lean, 12, 17 + sway + lean, 19, 4 + lean, 8]).fill({ color: 0x244447, alpha: 0.84 });
...
export function redrawAbyss(view: Graphics, width: number, height: number, elapsed: number) {
  ...
  view.poly([0, baseY + waveA, width * 0.16, baseY - 10 + waveB, width * 0.34, baseY + 3 - waveA, width * 0.52, baseY - 13 + waveB, width * 0.7, baseY + 1 + waveA, width * 0.86, baseY - 8 - waveB, width, baseY + waveA, width, height, 0, height])
    .fill({ color: Palette.magentaDeep, alpha: 0.78 });
  view.poly([0, baseY + 18, width * 0.2, baseY + 3 + waveB, width * 0.45, baseY + 12 - waveA, width * 0.72, baseY - 3 + waveA, width, baseY + 12, width, height, 0, height])
    .fill({ color: Palette.magenta, alpha: 0.2 });

  for (let i = 0; i < 5; i += 1) {
    const x = ((i * 73 + elapsed * 12) % (width + 40)) - 20;
    const y = baseY - 12 - ((i * 29 + elapsed * 17) % 76);
    const size = 3 + (i % 3) * 2;
    view.poly([x, y - size, x + size, y, x, y + size, x - size, y]).fill({ color: Palette.magenta, alpha: 0.15 });
  }
}

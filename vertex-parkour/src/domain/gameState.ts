...
export const createInitialState = (): GameState => ({
  ...
  dashUpgradeLevel: 0, flowUpgradeLevel: 0, skills: createEmptySkillLevels(),
});
...
export function tickState(state: GameState, deltaSeconds: number): GameState {
  ...
  if (state.landingTime > 0) { if (landingTime <= 0) velocityY = AUTO_JUMP_VELOCITY * Math.pow(REBOUND_JUMP_MULTIPLIER, state.skills.rebound); else velocityY = 0; }
...
export function applyDroneKill(state: GameState): GameState { if (state.gameOver) return state; return { ...state, dashReady: true, velocityY: Math.min(state.velocityY, DRONE_BOUNCE_VELOCITY), score: state.score + 400, flow: Math.min(12, state.flow + 1.8 + state.skills['kill-refund'] * KILL_REFUND_FLOW_BONUS) }; }

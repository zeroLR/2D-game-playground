...
import { getFlowGameplayModifiers, FLOW_MAX } from '../systems/FlowSystem';
...
export function tickState(state: GameState, deltaSeconds: number, tuning: DevTuning = DEFAULT_DEV_TUNING): GameState {
  ...
  const landingRecovery = getFlowGameplayModifiers(state.flow).landingRecoveryMultiplier;
  const landingTime = Math.max(0, state.landingTime - verticalDelta / landingRecovery);
...
export function applyLanding(state: GameState, platformY: number): GameState { if (state.gameOver) return state; return { ...state, playerY: platformY - PLAYER_FEET_OFFSET, velocityY: 0, landingTime: LANDING_DELAY, dashReady: true, wallSide: 0, flow: Math.min(FLOW_MAX, state.flow + 0.25) }; }

...
export class WorldState {
  ...
  commitRouteChoice(choiceId: number, selectedId: EntityId) { for (const route of this.routes) { if (route.choiceId !== choiceId) continue; if (route.id === selectedId) { route.taken = true; this.pendingRoute = route.kind; } else route.locked = true; } }
  consumePendingRoute() { const route = this.pendingRoute; this.pendingRoute = null; return route; }
  clear() { this.platforms.length = 0; this.crystals.length = 0; this.drones.length = 0; this.hazards.length = 0; this.spikes.length = 0; this.walls.length = 0; this.upgrades.length = 0; this.routes.length = 0; this.pendingRoute = null; this.nextId = 1; }

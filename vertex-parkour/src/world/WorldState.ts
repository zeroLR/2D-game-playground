...
export class WorldState {
  ...
  addSpawn(spawn: WorldSpawn, routeTheme: RouteKind | null = null): WorldEntity {
    ...
    switch (spawn.type) {
      case 'platform': {
        const entity: PlatformEntity = { id, type: 'platform', x: spawn.x, y: spawn.y, width: spawn.width, motion: spawn.motion, routeTheme };
        this.platforms.push(entity);
        return entity;
      }
      case 'crystal': {
        const entity: CrystalEntity = { id, type: 'crystal', x: spawn.x, y: spawn.y, taken: false };
        this.crystals.push(entity);
        return entity;
      }
      case 'drone': {
        const entity: DroneEntity = { id, type: 'drone', x: spawn.x, y: spawn.y, destroyed: false, phase: spawn.phase };
        this.drones.push(entity);
        return entity;
      }
      case 'hazard': {
      ...
      case 'upgrade': {
        const entity: UpgradeEntity = { id, type: 'upgrade', x: spawn.x, y: spawn.y, kind: spawn.kind, skillId: spawn.skillId, choiceId: spawn.choiceId, taken: false, locked: false };
        this.upgrades.push(entity);
        return entity;
      }
      case 'route': {
        const entity: RouteEntity = { id, type: 'route', x: spawn.x, y: spawn.y, kind: spawn.kind, choiceId: spawn.choiceId, taken: false, locked: false };
  ...
  commitRouteChoice(choiceId: number, selectedId: EntityId) {
    for (const route of this.routes) {
      if (route.choiceId !== choiceId) continue;
      if (route.id === selectedId) {
        route.taken = true;
        this.activeRoute = route.kind;
        this.pendingRoute = route.kind;
      } else route.locked = true;
    }
  }
  ...
  all(): WorldEntity[] {
    return [...this.platforms, ...this.crystals, ...this.drones, ...this.hazards, ...this.spikes, ...this.walls, ...this.upgrades, ...this.routes];
  }
}

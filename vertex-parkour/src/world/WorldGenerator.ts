...
export type BreakableSpawn = { type: 'breakable'; x: number; y: number; width: number; height: number };
export type WallSpawn = { type: 'wall'; side: -1 | 1; y: number; height: number };
export type WorldSpawn = PlatformSpawn | CrystalSpawn | DroneSpawn | HazardSpawn | SpikeSpawn | BreakableSpawn | WallSpawn;

export type WorldBand = { index: number; y: number; rest: boolean; spawns: WorldSpawn[] };
...
export class WorldGenerator {
  ...
  reset() { this.bandIndex = 0; this.lastY = START_PLATFORM_Y; this.previousPlatformLane = 180; }
  ...
  nextBand(): WorldBand {
    ...
    const spawns: WorldSpawn[] = [{ type: 'platform', x: platformLane, y: this.lastY, width: platformWidth }];

    if (rest) {
      if (this.random.next() < 0.42) spawns.push({ type: 'crystal', x: platformLane, y: this.lastY - 46 });
    ...
    if (this.bandIndex % 7 === 0 && this.bandIndex % 5 !== 0) {
      spawns.push({ type: 'breakable', x: routeLane, y: this.lastY - 38, width: 42, height: 54 });
    } else if (this.bandIndex % 5 === 0) {
    ...
    if (this.bandIndex % 5 !== 0 && platformWidth >= MIN_SPIKE_PLATFORM_WIDTH && this.random.next() < 0.24) {
      ...
      const side: -1 | 1 = approachDirection === 0 ? (this.random.next() < 0.5 ? -1 : 1) : (approachDirection as -1 | 1);
    ...
    if (this.random.next() < 0.48) spawns.push({ type: 'crystal', x: platformLane, y: this.lastY - 46 });
    return { index: this.bandIndex, y: this.lastY, rest, spawns };
  }

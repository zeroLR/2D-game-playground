...
describe('Biome foundation', () => {
  it('progresses through Chapter 1 and terminates at Storm Crown', () => {
    expect(STARTING_BIOME).toBe('teal-ruins');
    expect(BIOME_SEQUENCE).toEqual(['teal-ruins', 'amber-district', 'violet-zone', 'pale-heights', 'storm-crown']);
    ...
    expect(nextBiome('pale-heights')).toBe('storm-crown');
    expect(nextBiome('storm-crown')).toBe('storm-crown');
  });
  ...
  it('snapshots biome identity on already generated platforms', () => {
    ...
    world.setActiveBiome('amber-district'); const amber = world.addSpawn({ type: 'platform', x: 180, y: 400, width: 100 });
    world.setActiveBiome('violet-zone'); const violet = world.addSpawn({ type: 'platform', x: 180, y: 300, width: 100 });
    world.setActiveBiome('pale-heights'); const pale = world.addSpawn({ type: 'platform', x: 180, y: 200, width: 100 });
    world.setActiveBiome('storm-crown'); const storm = world.addSpawn({ type: 'platform', x: 180, y: 100, width: 100 });
    ...
    expect(storm).toMatchObject({ type: 'platform', biomeTheme: 'storm-crown' });
  });
  ...
  it('keeps all biome palette identities visually distinct', () => {
    ...
    const pale = getBiomeTheme('pale-heights'); const storm = getBiomeTheme('storm-crown');
    expect(mixTint(pale.platformTint, storm.platformTint, 0.5)).not.toBe(pale.platformTint);
    expect(mixTint(pale.platformTint, storm.platformTint, 0.5)).not.toBe(storm.platformTint);
  });

  it('resets biome progression with the world', () => {
    const world = new WorldState(); world.setActiveBiome('storm-crown'); world.clear();

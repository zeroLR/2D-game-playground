...
interface Enemy { g: Graphics; hp: number; speed: number; x: number; y: number; attack: number; heavy: boolean; flash: number; knockback: number; }
...
function updateEnemies(dt:number){const floor=gy();for(let i=enemies.length-1;i>=0;i--){const e=enemies[i];e.flash=Math.max(0,e.flash-dt);e.g.tint=e.flash>0?0xffd7e6:0xffffff;e.x+=e.speed*dt+e.knockback*dt;e.knockback*=Math.pow(.02,dt);const touchingPlayer=Math.abs(e.x-state.playerX)<(e.heavy?38:28)&&state.playerY>floor-54;if(touchingPlayer&&state.hitCooldown<=0){state.playerHp=Math.max(0,state.playerHp-(e.heavy?22:12));state.hitCooldown=BALANCE.player.hitInvulnerability;const kb:Facing=e.x<state.playerX?1:-1;state.playerX+=kb*(e.heavy?72:50);e.x-=kb*14;feedback.playerHit(state.playerX,state.playerY-35,e.heavy);}if(e.x>cx()-58){state.coreHp-=e.attack*dt;e.x-=8*dt;}e.g.position.set(e.x,floor);if(e.hp<=0){feedback.death(e.x,e.y-24,e.heavy);state.credits+=BALANCE.economy.killReward;state.killed++;e.g.destroy();enemies.splice(i,1);}}}

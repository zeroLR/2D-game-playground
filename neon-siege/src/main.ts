...
const PROJECTILE_SIMULATION_MARGIN = 120;
...
function segmentHitsEnemy(x1:number,y1:number,x2:number,y2:number,e:Enemy){const halfW=e.heavy?32:22,halfH=e.heavy?42:38,centerY=e.y-27,minX=Math.min(x1,x2),maxX=Math.max(x1,x2);if(maxX<e.x-halfW||minX>e.x+halfW)return false;const dx=x2-x1,dy=y2-y1,t=dx===0?0:Math.max(0,Math.min(1,(e.x-x1)/dx)),sampleX=x1+dx*t,sampleY=y1+dy*t;return Math.abs(sampleX-e.x)<=halfW&&Math.abs(sampleY-centerY)<=halfH;}
function updateBullets(dt:number){for(let i=bullets.length-1;i>=0;i--){const b=bullets[i],previousX=b.x,previousY=b.y;b.x+=b.vx*dt;b.y+=b.vy*dt;b.g.position.set(b.x,b.y);feedback.trail(previousX,b.x,b.y,b.direction);for(const e of enemies){if(b.hitEnemies.has(e))continue;if(segmentHitsEnemy(previousX,previousY,b.x,b.y,e)){e.hp-=b.damage;e.flash=.08;e.knockback+=b.direction*(e.heavy?b.knockback*.55:b.knockback);b.hitEnemies.add(e);b.pierce--;feedback.hit(e.x,e.y-28,b.damage,e.heavy);if(b.pierce<=0)break;}}if(b.pierce<=0||b.x>app.renderer.width+PROJECTILE_SIMULATION_MARGIN||b.x<-PROJECTILE_SIMULATION_MARGIN||b.y<-PROJECTILE_SIMULATION_MARGIN||b.y>app.renderer.height+PROJECTILE_SIMULATION_MARGIN){b.g.destroy();bullets.splice(i,1);}}}

...
export function connectorFor(width:number,x:number,lane:LaneId,direction:-1|1){const target=lane+direction;if(target<0||target>2)return undefined;return CONNECTORS.map(c=>({...c,px:c.x*width})).filter(c=>Math.abs(c.px-x)<=72&&((c.from===lane&&c.to===target)||(c.to===lane&&c.from===target))).sort((a,b)=>Math.abs(a.px-x)-Math.abs(b.px-x))[0];}

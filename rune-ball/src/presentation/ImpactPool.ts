...
export type ImpactTier = 'hit' | 'break';
...
export class ImpactPool extends Container {
  ...
  spawn(position: Point2D, tier: ImpactTier): void {
    const count = tier === 'break' ? 11 : 4;
    const speed = tier === 'break' ? 190 : 105;
    const lifetime = tier === 'break' ? 0.26 : 0.13;
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      particle.graphic.scale.set(tier === 'break' ? 1 : 0.65);
      particle.graphic.tint = index % 3 === 0 ? 0xd756ff : 0x6fe9ff;
      particle.graphic.alpha = tier === 'break' ? 0.95 : 0.78;

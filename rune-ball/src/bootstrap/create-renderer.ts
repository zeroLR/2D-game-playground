...
export async function createRenderer(resizeTo: HTMLElement): Promise<Application> {
  ...
  for (const attempt of attempts) {
    ...
    try {
      console.info(`[Rune Ball] Initializing ${attempt.label} renderer.`);
      await withTimeout(
        candidate.init({ ...commonOptions, ...attempt.options }),
        RENDERER_TIMEOUT_MS,
        `${attempt.label} renderer initialization`,
      );
      console.info(`[Rune Ball] ${attempt.label} renderer ready.`);
      return candidate;
    } catch (error) {
      lastError = error;
      console.warn(`[Rune Ball] ${attempt.label} renderer initialization failed.`, error);
      try {

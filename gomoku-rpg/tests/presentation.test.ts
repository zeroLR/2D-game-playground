...
describe('R5 skill bar presentation',()=>{
 it('marks skills the player cannot pay for as disabled',()=>{
  const bar=describeSkillBar(createCombatState(createBoard(),1),1,arcanist,createTurnState(),IDLE_TARGETING);
 ...
 it('disables every skill while the CPU holds the turn',()=>{
  const bar=describeSkillBar(createCombatState(createBoard(),5),1,arcanist,{turn:2,phase:'cpu',status:'playing'},IDLE_TARGETING);
...
describe('R5 status line',()=>{
 ...
 it('follows the turn phase',()=>{
  expect(statusKey(createTurnState(),IDLE_TARGETING)).toBe('yourTurn');
  expect(statusKey({turn:2,phase:'cpu',status:'playing'},IDLE_TARGETING)).toBe('opponentTurn');
  expect(statusKey(endMatch(createTurnState(),'victory'),IDLE_TARGETING)).toBe('victory');
 });
 it('names the selected skill and then asks for a destination',()=>{
  const opened=beginTargeting('blink');
  expect(statusKey(createTurnState(),opened)).toBe('blink');
  const picked=selectTargetingCell(state,1,opened,{row:4,col:4});
  if(picked.kind!=='source')throw new Error('expected a source selection');
  expect(statusKey(createTurnState(),picked.targeting)).toBe('selectDestination');
 });
 it('resolves every status key to a message in both locales',()=>{
  const keys=[statusKey(createTurnState(),IDLE_TARGETING),statusKey(createTurnState(),beginTargeting('charge')),'selectDestination','victory','defeat','draw'] as const;
  keys.forEach((key)=>{expect(typeof getMessages('en')[key]).toBe('string');expect(typeof getMessages('zh-TW')[key]).toBe('string');});
 });
});

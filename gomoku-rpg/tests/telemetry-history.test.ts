...
describe('playtest history',()=>{
  it('persists every completed match',()=>{const first=createPlaytestMetrics('vanguard');finishMetrics(first,'victory',2);const second=createPlaytestMetrics('shade');finishMetrics(second,'defeat',1);expect(loadHistory()).toHaveLength(2);expect(exportHistory().heroCounts).toEqual({vanguard:1,arcanist:0,shade:1});});

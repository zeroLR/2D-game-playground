...
describe('V2 hero ability economy contract',()=>{
 it('declares an economy as part of every hero identity',()=>{for(const hero of Object.values(heroes))expect(hero.abilityEconomy).toEqual({kind:'resource',resourceId:'mana',max:5});});
 it('moves authoritative activation cost into activation metadata',()=>{expect(skills.blink.activation).toEqual({kind:'resource',resourceId:'mana',amount:2});expect(skills.charge.activation).toEqual({kind:'resource',resourceId:'mana',amount:3});expect(skills.lattice.activation).toEqual({kind:'resource',resourceId:'mana',amount:3});});
});

...
describe('V2 hero ability economy contract',()=>{
 it('migrates Vanguard to cooldown while keeping the remaining roster on Mana for now',()=>{expect(heroes.vanguard.abilityEconomy).toEqual({kind:'cooldown'});for(const id of ['arcanist','shade','architect'] as const)expect(heroes[id].abilityEconomy).toEqual({kind:'resource',resourceId:'mana',max:5});});
 it('resolves shared Blink differently by hero economy',()=>{expect(resolveAbilityActivation('vanguard','blink')).toEqual({kind:'cooldown',turns:3});expect(resolveAbilityActivation('arcanist','blink')).toEqual({kind:'resource',resourceId:'mana',amount:2});});

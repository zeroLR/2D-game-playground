...
import { chargeSkill,commonSkillIds,corruptSkill,phaseSkill,sealSkill } from '../src/skills';
...
describe('M2.4 Shade Corrupt',()=>{it('targets only enemy stones adjacent to a Shade stone',()=>{const board=createBoard();board[4][4]=1;board[4][5]=2;board[1][1]=2;expect(corruptSkill.legalTargets({state:createCombatState(board,5),player:1})).toEqual([{row:4,col:5}]);});});
...
describe('M2.4 Vanguard Charge',()=>{
 ...
 it('pushes an enemy one additional step in the same direction',()=>{const board=createBoard();board[4][3]=1;board[4][4]=2;const next=chargeSkill.execute({state:createCombatState(board,3),player:1},{row:4,col:4},{row:4,col:3});expect(next.board[4][4]).toBe(1);expect(next.board[4][5]).toBe(2);});
});

describe('M2.4 Arcanist Phase',()=>{
 it('is explicitly a free action costing 3 Mana',()=>{expect(phaseSkill.consumesTurn).toBe(false);expect(phaseSkill.cost).toBe(3);});
 it('leaps over exactly one occupied cell into the empty cell behind it',()=>{const board=createBoard();board[4][3]=1;board[4][4]=2;const state=createCombatState(board,3);expect(phaseSkill.legalTargets({state,player:1},{row:4,col:3})).toContainEqual({row:4,col:5});const next=phaseSkill.execute({state,player:1},{row:4,col:5},{row:4,col:3});expect(next.board[4][3]).toBe(0);expect(next.board[4][4]).toBe(2);expect(next.board[4][5]).toBe(1);expect(getMana(next,1)).toBe(0);});
 it('cannot phase without an occupied middle cell',()=>{const board=createBoard();board[4][3]=1;expect(phaseSkill.legalTargets({state:createCombatState(board,3),player:1},{row:4,col:3})).not.toContainEqual({row:4,col:5});});
});

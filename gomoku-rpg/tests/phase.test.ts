...
describe('M2.4d Arcanist Flame prototype',()=>{
 it('costs 3 Mana and consumes the turn',()=>{expect(phaseSkill.consumesTurn).toBe(true);expect(phaseSkill.cost).toBe(3);expect(phaseSkill.activation).toEqual({kind:'resource',resourceId:'mana',amount:3});});
 ...
 it('places a stone and flames the four orthogonal empty neighbours',()=>{const target={row:4,col:4};const result=resolveSkillAction(createCombatState(createBoard(),3),'arcanist',1,'phase',target);expect(result.ok).toBe(true);expect(result.state.board[4][4]).toBe(1);expect(getMana(result.state,1)).toBe(1);expect(result.state.flames.map((f)=>f.pos)).toEqual(expect.arrayContaining([{row:3,col:4},{row:5,col:4},{row:4,col:3},{row:4,col:5}]));expect(result.state.flames).toHaveLength(4);});

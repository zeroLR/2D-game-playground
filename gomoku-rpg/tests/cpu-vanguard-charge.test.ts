...
describe('M2.5 Slice 2 CPU Vanguard Charge',()=>{
 ...
 it('uses Charge when pushing a player threat is more valuable than ordinary placement',()=>{const board=createBoard();board[4][1]=1;board[4][2]=1;board[4][3]=1;board[3][4]=2;const state=setMana(createCombatState(board),2,3);const result=resolveCpuTurn(state,'vanguard');expect(result.action?.kind).toBe('skill');if(result.action?.kind==='skill')expect(result.action.skillId).toBe('charge');expect(result.state.resources[2].mana).toBe(0);});

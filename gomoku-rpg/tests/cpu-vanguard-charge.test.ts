...
describe('V2 CPU Vanguard cooldown economy',()=>{
 ...
 it('never exposes Charge to a non-Vanguard CPU',()=>{const board=createBoard();board[4][4]=2;const state=setMana(createCombatState(board),2,5);for(const hero of ['arcanist','shade','architect','swordmaster'] as const){const candidates=cpuActionCandidates(state,hero);expect(candidates.some((a)=>a.kind==='skill'&&a.skillId==='charge')).toBe(false);const result=resolveCpuTurn(state,hero);if(result.action?.kind==='skill')expect(result.action.skillId).not.toBe('charge');}});
 it('supports deterministic CPU hero selection for the expanded roster',()=>{expect(randomCpuHero(()=>0)).toBe('vanguard');expect(randomCpuHero(()=>0.99)).toBe('swordmaster');});
});

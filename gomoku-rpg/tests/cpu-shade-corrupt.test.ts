...
describe('M2.5 Slice 4 CPU Shade Corrupt',()=>{
 ...
 it('uses Corrupt to dismantle a high-value player structure',()=>{const board=createBoard();board[4][2]=1;board[4][3]=1;board[4][4]=1;board[3][3]=2;const state=setMana(createCombatState(board),2,3);const result=resolveCpuTurn(state,'shade');expect(result.action?.kind).toBe('skill');if(result.action?.kind==='skill'){expect(result.action.skillId).toBe('corrupt');expect(result.action.target).toEqual({row:4,col:3});expect(result.state.board[4][3]).toBe(0);}expect(getMana(result.state,2)).toBe(0);});
 it('keeps Mana when ordinary placement is more valuable than Corrupt',()=>{const board=createBoard();board[4][1]=2;board[4][2]=2;board[4][3]=2;board[2][2]=1;board[3][2]=2;const state=setMana(createCombatState(board),2,3);const result=resolveCpuTurn(state,'shade');expect(result.action?.kind).toBe('place');expect(getMana(result.state,2)).toBeGreaterThanOrEqual(3);});
});

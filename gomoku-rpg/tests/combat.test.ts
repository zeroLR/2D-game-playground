...
describe('M1 persistent skills',()=>{
 it('Blink moves a stone and activation spends Mana',()=>{const b=createBoard();b[4][4]=1;const r=resolveSkillAction(createCombatState(b,2),'vanguard',1,'blink',{row:5,col:5},{row:4,col:4});expect(r.ok).toBe(true);expect(r.state.board[4][4]).toBe(0);expect(r.state.board[5][5]).toBe(1);expect(getMana(r.state,1)).toBe(0);});
...
describe('M2.4 Vanguard Bulwark',()=>{
 ...
 it('guards the target and adjacent friendly cluster for 3 Mana',()=>{const b=createBoard();b[4][4]=1;b[4][5]=1;b[5][5]=1;b[7][7]=1;const r=resolveSkillAction(createCombatState(b,3),'vanguard',1,'bulwark',{row:4,col:4});expect(r.ok).toBe(true);expect(isGuarded(r.state,{row:4,col:4})).toBe(true);expect(isGuarded(r.state,{row:4,col:5})).toBe(true);expect(isGuarded(r.state,{row:5,col:5})).toBe(true);expect(isGuarded(r.state,{row:7,col:7})).toBe(false);expect(getMana(r.state,1)).toBe(0);});

...
describe('M2.5 CPU action evaluator',()=>{
 ...
 it('never lets Lv4 random candidate selection ignore a forced block',()=>{const board=createBoard();board[3][0]=2;board[4][1]=1;board[4][2]=1;board[4][3]=1;board[4][4]=1;const state=createCombatState(board),candidates=cpuPlaceCandidates(state);const selected=chooseCpuAction(state,candidates,cpuDifficulty(4),()=>0.999);expect(selected?.action).toEqual({kind:'place',at:{row:4,col:5}});});
 it('never lets Lv4 heuristic scoring override an immediate CPU win',()=>{const board=createBoard();board[2][1]=2;board[2][2]=2;board[2][3]=2;board[2][4]=2;board[4][3]=1;board[4][4]=1;board[4][5]=1;const state=createCombatState(board),candidates=cpuPlaceCandidates(state);const selected=chooseCpuAction(state,candidates,cpuDifficulty(4),()=>0.999);expect(selected?.action).toEqual({kind:'place',at:{row:2,col:5}});});

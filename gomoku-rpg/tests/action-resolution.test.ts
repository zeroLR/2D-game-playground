...
describe('R1 place resolution',()=>{
 ...
 it('applies the hero passive as part of the resolution',()=>{const b=createBoard();b[4][2]=b[4][3]=1;const at={row:4,col:4};const r=resolvePlaceAction(createCombatState(b),'vanguard',1,at);expect(r.passiveTriggered).toBe(true);expect(isGuarded(r.state,at)).toBe(true);});
...
describe('R1 skill resolution',()=>{
 it('resolves Blink through source and target',()=>{const b=createBoard();b[4][4]=1;const r=resolveSkillAction(createCombatState(b,2),'vanguard',1,'blink',{row:5,col:5},{row:4,col:4});expect(r.ok).toBe(true);expect(r.state.board[4][4]).toBe(0);expect(r.state.board[5][5]).toBe(1);expect(getMana(r.state,1)).toBe(0);});
 ...
 it('rejects a skill the caster cannot pay for',()=>{const b=createBoard();b[4][4]=1;const r=resolveSkillAction(createCombatState(b,1),'vanguard',1,'blink',{row:5,col:5},{row:4,col:4});expect(r.ok).toBe(false);expect(r.error).toBe('insufficient-mana');});
...
describe('R1 turn consumption',()=>{
 it('reports consumedTurn from the skill definition',()=>{
  const b=createBoard();b[4][4]=1;b[4][5]=2;
  expect(resolveSkillAction(createCombatState(b,3),'vanguard',1,'blink',{row:5,col:5},{row:4,col:4}).consumedTurn).toBe(true);
 ...
 it('declares every shipped skill as turn consuming until a free action exists',()=>{Object.values(skills).forEach((skill)=>expect(skill.consumesTurn).toBe(true));});
});

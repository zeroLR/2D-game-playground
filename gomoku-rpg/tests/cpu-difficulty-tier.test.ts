...
describe('M7.3-R difficulty tier foundation',()=>{
  ...
  it('maps tiers to strictly increasing hidden profiles',()=>{const levels=CPU_DIFFICULTIES.map(cpuProfileLevelForDifficulty);expect(levels).toEqual([5,20,30,50,70,90]);for(let i=1;i<levels.length;i++)expect(levels[i]).toBeGreaterThan(levels[i-1]);});
  ...
  it('maps legacy numeric records to the closest named tier',()=>{expect(closestDifficultyForProfileLevel(3)).toBe('easy');expect(closestDifficultyForProfileLevel(20)).toBe('normal');expect(closestDifficultyForProfileLevel(48)).toBe('extreme');expect(closestDifficultyForProfileLevel(100)).toBe('chaos');});
});

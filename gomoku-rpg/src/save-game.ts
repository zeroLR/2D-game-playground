...
export function normalizeGameSave(value:unknown):GameSaveV1{
  ...
  const completed=Array.isArray(story.completedEncounterIds)?story.completedEncounterIds.filter((id):id is StoryEncounterId=>['E1-1','E1-2','E1-3','E1-4','E1-5','E1-BOSS'].includes(String(id))):[];
  const heroes=Array.isArray(progression.unlockedHeroes)?progression.unlockedHeroes.filter((id):id is HeroId=>['vanguard','arcanist','shade'].includes(String(id))):['vanguard'];

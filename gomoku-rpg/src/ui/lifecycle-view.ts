...
export function renderPauseOverlay(ctx:ViewContext,opts:{onResume:()=>void;onRestart:()=>void;onQuit:()=>void}){const shade=new Graphics().rect(0,0,390,844).fill({color:0x15141b,alpha:.72});ctx.root.addChild(shade);const card=new Graphics().roundRect(36,206,318,380,12).fill(0xf8f5ef).stroke({color:edge,width:1});ctx.root.addChild(card);centeredLabel(ctx,ctx.locale==='en'?'PAUSED':'暫停',CANVAS_CENTER,242,25,ink,'700');action(ctx,ctx.locale==='en'?'Resume':'繼續對戰',302,opts.onResume,true);action(ctx,ctx.locale==='en'?'Restart':'重新開始',408,opts.onRestart);button(ctx,70,520,250,48,opts.onQuit);centeredLabel(ctx,ctx.locale==='en'?'Surrender / Main Menu':'放棄對局 / 返回主選單',CANVAS_CENTER,535,13,0xa4473d,'700');}

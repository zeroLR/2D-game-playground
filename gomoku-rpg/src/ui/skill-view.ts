...
export interface SkillBarProps{items:SkillBarItem[];onSelect:(skillId:SkillId)=>void;followUpOpen?:boolean;onEndFollowUp?:()=>void}
...
function activationText(ctx:ViewContext,item:SkillBarItem){
  ...
  if(item.activation.kind==='resource'&&item.activation.resourceId==='momentum')return ctx.locale==='en'?`MOMENTUM ${item.resourceCurrent}/${item.resourceMax} · COST ${item.activation.amount}`:`劍勢 ${item.resourceCurrent}/${item.resourceMax} · 消耗 ${item.activation.amount}`;
...
export function renderSkillBar(ctx:ViewContext,{items,onSelect,followUpOpen=false,onEndFollowUp}:SkillBarProps){
  const m=getMessages(ctx.locale);
  ...
  const selected=items.find((item)=>item.selected);if(selected){const help=label(ctx,m[selected.descriptionKey as keyof Messages] as string,24,739,8,muted);help.style.wordWrap=true;help.style.wordWrapWidth=followUpOpen?210:342;}
  if(followUpOpen&&onEndFollowUp){button(ctx,250,734,116,34,onEndFollowUp);centeredLabel(ctx,ctx.locale==='en'?'END TURN':'結束回合',308,744,8,ink,'700');}
}

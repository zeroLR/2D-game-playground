...
export function createMatchRuntime({heroId,schedule,cancel,onChange,onEvent,initialState=()=>createCombatState(createBoard())}:MatchRuntimeOptions):MatchRuntime{
 let hero=heroId;let loadout=createLoadout(hero);let combat=initialState();let metrics=createPlaytestMetrics(hero,getMana(combat,PLAYER));let turn=createTurnState();let targeting:SkillTargetingState=IDLE_TARGETING;let cpuTimer:Timer|null=null;let turnStartRecorded=false;
 ...
 function reset(){clearCpuTimer();combat=initialState();metrics=createPlaytestMetrics(hero,getMana(combat,PLAYER));turn=createTurnState();targeting=IDLE_TARGETING;turnStartRecorded=false;onEvent({kind:'reset'});}

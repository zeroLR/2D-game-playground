...
export interface CpuResolution{outcome:CpuOutcome;state:CombatState;at:Pos|null;action:CpuAction|null;passiveTriggered:boolean;cpuLevel:number;decisionScore:number|null;decisionRegret:number|null}
...
export function resolveCpuTurn(state:CombatState,heroId:HeroId='arcanist',cpuLevel:number=CPU_BASELINE_LEVEL):CpuResolution{
 const profile=cpuDifficulty(cpuLevel),selected=chooseCpuAction(state,cpuActionCandidates(state,heroId),profile);const meta={cpuLevel:profile.level,decisionScore:selected?.score??null,decisionRegret:selected?.regret??null};if(!selected)return {outcome:'draw',state,at:null,action:null,passiveTriggered:false,...meta};const action=selected.action;

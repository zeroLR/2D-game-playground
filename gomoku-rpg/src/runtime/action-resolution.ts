...
export function resolveSkillAction(state:CombatState,heroId:HeroId,player:Player,skillId:SkillId,target:Pos,source?:Pos):ActionResolution{
  ...
  const readiness=canActivate(state,player,activation,skillId);if(!readiness.ready)return rejected(state,target,readiness.reason==='insufficient-resource'?'insufficient-mana':'ability-unavailable',skillId);

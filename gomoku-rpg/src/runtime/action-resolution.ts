...
import { heroUsesMana, prepareHeroAbilityState, resolveAbilityActivation } from '../hero-ability-activation';
...
export function resolveSkillAction(state:CombatState,heroId:HeroId,player:Player,skillId:SkillId,target:Pos,source?:Pos):ActionResolution{
  ...
  const activation=resolveAbilityActivation(heroId,skillId),prepared=prepareHeroAbilityState(state,heroId,player,skillId);
  const readiness=canActivate(prepared,player,activation,skillId);
  if(!readiness.ready){
  ...
  const context={state:prepared,player};if(skill.legalSources&&(!source||!isLegalPosition(source,skill.legalSources(context))))return rejected(state,target,'invalid-target',skillId);if(!isLegalPosition(target,skill.legalTargets(context,source)))return rejected(state,target,'invalid-target',skillId);
  const activated=consumeActivation(prepared,player,activation,skillId);

...
export function finishMetrics(metrics:PlaytestMetrics,outcome:Exclude<MatchOutcome,'playing'>,mana:number){if(metrics.outcome!=='playing')return;metrics.outcome=outcome;metrics.endedAt=Date.now();recordMana(metrics,mana);saveCompletedMatch(metrics);showClearControl();}
...
// Existing result-screen METRICS action now exports the complete persisted dataset.
export function summaryText(_metrics:PlaytestMetrics){return historyText();}
function showClearControl(){if(typeof document==='undefined'||document.getElementById(CLEAR_BUTTON_ID))return;const button=document.createElement('button');button.id=CLEAR_BUTTON_ID;button.textContent='CLEAR METRICS';button.setAttribute('aria-label','Clear all Gomoku RPG playtest metrics');Object.assign(button.style,{position:'fixed',right:'12px',bottom:'12px',zIndex:'20',padding:'8px 10px',border:'1px solid #c9a35c',borderRadius:'6px',background:'#292936',color:'#f3e6c9',font:'10px Georgia, serif',letterSpacing:'1px',opacity:'.82'});button.onclick=()=>{if(window.confirm('Clear all saved playtest match history?')){clearHistory();button.textContent='CLEARED';setTimeout(()=>{button.textContent='CLEAR METRICS';},900);}};document.body.appendChild(button);}

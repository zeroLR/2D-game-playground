...
function updateMomentum(state:CombatState,player:Player,patternReward:number,preserve=false):PassiveResult{const before=getAbilityResource(state,player,'momentum');const after=patternReward>0?Math.min(3,before+patternReward):preserve?before:Math.max(0,before-1);if(after===before)return {state,triggered:false};return {state:setAbilityResource(state,player,'momentum',after),triggered:true,resourceGained:{resourceId:'momentum',amount:after-before}};}
...
export function applyAfterPlacePassive(state:CombatState,heroId:HeroId,player:Player,placed:Pos,manaGained:number,options:AfterPlacePassiveOptions={}):PassiveResult {
  ...
  /** Momentum: qualifying 3/4-line placements gain reward; quiet placements decay unless Flow Step preserves the chain. */

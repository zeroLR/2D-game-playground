import { getAbilityResource,setAbilityResource,type ResourceId } from './ability-economy';
...
function updateMomentum(state:CombatState,player:Player,patternReward:number):PassiveResult{const before=getAbilityResource(state,player,'momentum');const after=patternReward>0?Math.min(3,before+patternReward):Math.max(0,before-1);if(after===before)return {state,triggered:false};return {state:setAbilityResource(state,player,'momentum',after),triggered:true,resourceGained:{resourceId:'momentum',amount:after-before}};}
...
export function applyAfterPlacePassive(state:CombatState,heroId:HeroId,player:Player,placed:Pos,manaGained:number):PassiveResult {
  ...
  /** Momentum: qualifying 3/4-line placements gain the pattern reward; quiet placements decay the chain by 1. */
  if(passive==='momentum')return updateMomentum(state,player,manaGained);

...
const messages = {
  en: {
    ...
    costMana: (cost: number) => `COST  ${cost} MANA`, blinkSelect: 'Select one of your stones, then an empty cell.',
    blinkHelp: 'Move one of your stones. Uses your whole turn.', guardHelp: 'Protect one stone from one hostile skill effect.', sealHelp: 'Block one empty intersection for one opponent turn.',
    ...
    vanguard: 'VANGUARD', arcanist: 'ARCANIST', shade: 'SHADE', defense: 'DEFENSE', control: 'CONTROL', disruption: 'DISRUPTION',
    fortified: 'FORTIFIED', flow: 'FLOW', pressure: 'PRESSURE', passive: 'PASSIVE', skillsLabel: 'SKILLS',
    fortifiedHelp: 'Pattern placement: the new stone gains Guard through the opponent turn.',
  },
  'zh-TW': {
    ...
    costMana: (cost: number) => `消耗 ${cost} 魔力`, blinkSelect: '選擇自己的一枚棋子，再選擇空位。',
    blinkHelp: '移動自己一枚棋子，會消耗整個回合。', guardHelp: '保護一枚棋，抵擋一次敵方技能效果。', sealHelp: '封鎖一個空位，直到對手回合結束。',
    ...
    vanguard: '先鋒', arcanist: '奧術師', shade: '影刃', defense: '防禦', control: '控制', disruption: '擾亂',
    fortified: '堅守', flow: '流轉', pressure: '壓迫', passive: '被動', skillsLabel: '技能',
    fortifiedHelp: '落子形成得魔力的棋型時，新落下的棋獲得守護直到對手回合結束。',
  },
} as const;
...
export function loadLocale(): Locale { try { const value=localStorage.getItem(LOCALE_STORAGE_KEY); return value==='zh-TW'||value==='en'?value:DEFAULT_LOCALE; } catch { return DEFAULT_LOCALE; } }
export function saveLocale(locale: Locale) { try { localStorage.setItem(LOCALE_STORAGE_KEY,locale); } catch { /* storage is optional */ } }

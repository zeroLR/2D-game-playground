...
export type CombatState={board:Cell[][];mana:number;resources:CombatResources;activePlayer:Player;guards:TimedPosition[];seals:TimedPosition[];echoes:TimedPosition[];corrupted:CorruptedPosition[]};
...
export function createCombatState(board:Cell[][],mana=0,activePlayer:Player=1):CombatState{return{board,mana,resources:{1:{mana},2:{mana:0}},activePlayer,guards:[],seals:[],echoes:[],corrupted:[]};}
...
/** Echoes expire after the opponent response, immediately before the owner's next actionable turn. */
export function expireEffectsAfterTurn(state:CombatState,player:Player):CombatState{const expiredEchoes=state.echoes.filter((e)=>e.expiresAfterPlayer===player);let board=state.board;if(expiredEchoes.length){board=state.board.map((row)=>[...row]);for(const echo of expiredEchoes)if(board[echo.pos.row]?.[echo.pos.col]===echo.owner)board[echo.pos.row][echo.pos.col]=0;}const corrupted=state.corrupted.map((e)=>({...e,turnsRemaining:e.turnsRemaining-1})).filter((e)=>e.turnsRemaining>0);return{...state,board,guards:state.guards.filter((e)=>e.expiresAfterPlayer!==player),seals:state.seals.filter((e)=>e.expiresAfterPlayer!==player),echoes:state.echoes.filter((e)=>e.expiresAfterPlayer!==player),corrupted};}
...
export function addEcho(state:CombatState,pos:Pos,owner:Player):CombatState{return{...state,echoes:[{pos,owner,expiresAfterPlayer:owner===1?2:1}]};}

...
export type CombatState={board:Cell[][];mana:number;resources:CombatResources;activePlayer:Player;guards:TimedPosition[];seals:TimedPosition[];echoes:PhaseEcho[];corrupted:CorruptedPosition[]};
...
/** Phase Echo survives the opponent response and the owner's next action, then its origin collapses. */

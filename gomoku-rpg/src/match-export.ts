...
export function exportMatchesCsv(records:readonly MatchRecord[]){
 const header=['id','startedAt','finishedAt','hero','playerLoadout','cpuHero','cpuLoadout','cpuDifficulty','cpuProfileLevel','result','turns','actions','playerSkillUses','cpuSkillUses'];
 const rows=records.map(r=>[r.id,r.startedAt,r.finishedAt,r.heroId,(r.playerSkillIds??[]).join('+'),r.cpuHeroId,(r.cpuSkillIds??[]).join('+'),r.cpuDifficulty??'',r.cpuProfileLevel??r.cpuLevel??'',r.result,r.turns,r.actions.length,totalSkillUses(r.metrics.skillUses),r.actions.filter(a=>a.actor==='cpu'&&a.kind==='skill').length]);

...
const GUARDRAIL_DIMENSIONS: Record<GuardrailProfile, GuardrailDimensions> = {
  // A low curb catches small drift but can still be overcome at high lateral speed.
  curb: { height: 0.32, thickness: 0.16 },
  // A full prototype safety rail is intentionally below eye height while still
  // high enough to catch the rolling sphere before it leaves the platform.
...
function rotateByEulerXYZ(vector: TrackVec3, rotation: TrackVec3): TrackVec3 {
  ...
  // Same XYZ quaternion convention used by Three.js and Cannon for authored
  // track pieces, so safety geometry stays local to banked / yawed surfaces.
...
function guardrailForSide(
  ...
  return {
    ...
    size: {
      ...
      y: dimensions.height,
      // Leave a very small opening at piece ends so adjacent turns do not form
      // protruding collision lips that can snag the sphere.
      z: Math.max(0.4, piece.size.z - 0.18),
    },
    rotation: { ...piece.rotation },
    // Moving safety geometry must remain kinematic with its parent. It is not
    // magnetic itself: cyan remains gameplay surface language, amber remains
    // moving geometry language.

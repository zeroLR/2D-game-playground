...
const DOMINO={thickness:.22,height:1.25,width:.72,spacing:.78};
const levels:Level[]=[
{name:'第一推',goal:'推倒整條骨牌',path:[[-4.2,0],[4.2,0]],count:12,accent:0xf08a72},
{name:'轉個彎',goal:'讓連鎖反應繞過轉角',path:[[-4,-1.8],[-.5,-1.8],[-.5,2.4]],count:12,accent:0x77bfa3},
{name:'蛇蛇走',goal:'沿 S 彎完成連鎖',path:[[-4,-1.2],[-2.4,1.1],[-.7,1.1],[1,-1.1],[2.6,-1.1],[4,1]],count:15,accent:0x7aa7d9},
{name:'滾球球',goal:'把球撞下終點',path:[[-4,0],[3.7,0]],count:11,accent:0xe8b75b,prop:'ball'},
{name:'叫醒貓',goal:'最後一張骨牌要碰到貓',path:[[-4,-.8],[-2,.6],[0,-.5],[2,.5],[3.7,0]],count:13,accent:0xc999d4,prop:'cat'},
{name:'雞飛狗跳',goal:'把連鎖送到逃跑的小雞',path:[[-4,-1.4],[-1.2,-1.4],[.5,-.2],[2.2,1.4],[3.8,1.4]],count:13,accent:0xf2cf66,prop:'chicken'},
{name:'繞圈圈',goal:'完成半圓連鎖',path:Array.from({length:13},(_,i)=>{const a=Math.PI+i*Math.PI/12;return[Math.cos(a)*3.4,Math.sin(a)*3.4-1] as Point}),count:15,accent:0x69c4cc},
{name:'叮一聲',goal:'讓最後的骨牌敲響鈴鐺',path:[[-4,-.7],[-2,0],[0,.7],[2,0],[3.8,.6]],count:13,accent:0xf09b65,prop:'bell'},
{name:'螺旋',goal:'從外圈一路倒進中心',path:Array.from({length:18},(_,i)=>{const a=i*.38,r=3.9-i*.15;return[Math.cos(a)*r,Math.sin(a)*r] as Point}),count:17,accent:0x94bd65},
{name:'飛高高',goal:'完成連鎖，把氣球送上天',path:[[-4,-.6],[-2,.5],[0,-.5],[2,.5],[3.8,0]],count:13,accent:0xe889a8,prop:'balloon'}
];

function samplePath(path:Point[],count:number):Point[]{
 const lengths:number[]=[];let total=0;
 for(let i=0;i<path.length-1;i++){const dx=path[i+1][0]-path[i][0],dz=path[i+1][1]-path[i][1];const len=Math.hypot(dx,dz);lengths.push(len);total+=len}
 const spacing=Math.max(DOMINO.spacing,total/(count-1));const actualCount=Math.floor(total/spacing)+1;
 return Array.from({length:actualCount},(_,i)=>{let d=Math.min(i*spacing,total);for(let s=0;s<lengths.length;s++){if(d<=lengths[s]||s===lengths.length-1){const t=lengths[s]?Math.min(1,d/lengths[s]):0;return[path[s][0]+(path[s+1][0]-path[s][0])*t,path[s][1]+(path[s+1][1]-path[s][1])*t] as Point}d-=lengths[s]}return path.at(-1)!});
}
function heading(points:Point[],i:number){const prev=points[Math.max(0,i-1)],next=points[Math.min(points.length-1,i+1)];return Math.atan2(next[0]-prev[0],next[1]-prev[1]);}
...
const world=new CANNON.World({gravity:new CANNON.Vec3(0,-9.82,0)});world.allowSleep=true;if(world.solver instanceof CANNON.GSSolver)world.solver.iterations=15;
...
world.addContactMaterial(new CANNON.ContactMaterial(dominoMat,dominoMat,{friction:.22,restitution:.01}));
...
function load(index:number){current=index;armed=false;clear();const l=levels[index];currentPoints=samplePath(l.path,l.count);(hud.querySelector('.title') as HTMLElement).textContent=`${index+1}. ${l.name}`;(hud.querySelector('.goal') as HTMLElement).textContent=l.goal;[...levelBar.children].forEach((x,i)=>x.classList.toggle('active',i===index));
 currentPoints.forEach((p,i)=>{const angle=heading(currentPoints,i);const mesh=new THREE.Mesh(new THREE.BoxGeometry(DOMINO.width,DOMINO.height,DOMINO.thickness),new THREE.MeshStandardMaterial({color:i===0?0xff6b5e:l.accent,roughness:.55}));mesh.position.set(p[0],DOMINO.height/2+.01,p[1]);mesh.rotation.y=angle;mesh.castShadow=true;scene.add(mesh);const shape=new CANNON.Box(new CANNON.Vec3(DOMINO.width/2,DOMINO.height/2,DOMINO.thickness/2));const body=new CANNON.Body({mass:1,shape,material:dominoMat,allowSleep:true,sleepSpeedLimit:.08,sleepTimeLimit:.25});body.position.set(p[0],DOMINO.height/2+.01,p[1]);body.quaternion.setFromEuler(0,angle,0);body.linearDamping=.16;body.angularDamping=.1;world.addBody(body);body.sleep();meshes.push(mesh);bodies.push(body)});addProp(l)}
...
function push(){if(armed||bodies.length<2)return;armed=true;bodies.forEach(b=>b.wakeUp());const first=bodies[0],second=bodies[1];const dx=second.position.x-first.position.x,dz=second.position.z-first.position.z,len=Math.hypot(dx,dz)||1,dirX=dx/len,dirZ=dz/len;first.applyImpulse(new CANNON.Vec3(dirX*.45,0,dirZ*.45),new CANNON.Vec3(0,.5,0));first.applyTorque(new CANNON.Vec3(dirZ*3.2,0,-dirX*3.2));}

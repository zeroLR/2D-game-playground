...
if(world.solver instanceof CANNON.GSSolver) world.solver.iterations=15;
...
function heading(points:[number,number][],i:number){const b=points[Math.min(i+1,points.length-1)],p=points[Math.max(0,i-1)];const dx=b[0]-p[0],dz=b[1]-p[1];return Math.atan2(dx,dz)}
